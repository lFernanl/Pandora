...
exports.tour = function(t) {
  if (typeof t != "undefined") var tour = t; else var tour = new Object();
        var tourStuff = {
                ...
                lideres: function() {
                        return ['hydrei','htscrag','cfrgvolbt','arag','darkomar','lfgjolteonskyrim','haayato','mrpoke','chjohncalderon','hgslashraccoon','elitalianochamp','chblue123','bsarticwind','bsemily3','mlpandrea','hgdark','cfmiguelcoro'];
                },
                cargos: function() {
                ...
                medallas: function() {
                        return ['Medalla Meda-Fuerza','Machinery Medal','Blaze Medal', 'Sweet Tear Medal', 'Medalla Helada', 'Black Belt Medal', '', '', 'Mind Medal', 'Chlorophyll Medal', '', '', 'Medalla Escama de Dragon', '', '', '', 'Failure Sand Medal', '', '', '', '', '', 'Campeon de la Liga Pokemon Hispano'];
                },
                altomando: function() {
                        return ['davidness','princeaugust','raichy','pokebasket'];
                },
                ...
                nextRound: function(rid) {
                        ...
                        if (w.length == 1) {
                                ...
                                /* if (rid === 'lobby') {
                                        var firstwin = tour[rid].size * 10;
                                }
                                var secondwin = firstwin / 2;
                                if (Users.users[w[0]]) {
                                        Users.users[w[0]].moneh += firstwin;
                                        Users.users[w[0]].prewritemoney();
                                }
                                if (Users.users[l[0]]) {
                                        Users.users[l[0]].moneh += secondwin;
                                        Users.users[l[0]].prewritemoney();
                                }
                               
                                if (rid === 'lobby') Rooms.rooms[rid].addRaw(Users.users[w[0]].name + ' ha ganado ' + firstwin + ' PokeDolares. ' + Users.users[l[0]].name + ' ha ganado ' + secondwin + ' PokeDolares.');
                                if (Rooms.global.lockdown) {
                                        Rooms.rooms[rid].addRaw('El servidor esta bajo arresto. Luego del reinicio del servidor, solicitar que la ganancia se haga efectiva.');
                                } else if (!config.avoidwrite) {
                                        Users.exportUserwealth();
                                } else if ( process.uptime() / 1200 > Users.savedmoney ) {
                                        Users.savedmoney++;
                                        Users.exportUserwealth();
                                } */
                                tour[rid].status = 0;
        ...
        if (this.league) {
                ...
                if (istie) {
                        ...
                        if (winnerid === this.challenger) {
                                ...
                                /* Users.get(winnerid).moneh += 15;
                                        Users.get(winnerid).prewritemoney();
                                        if (!config.avoidwrite) {
                                                Users.exportUserwealth();
                                        } else if ( process.uptime() / 1200 > Users.savedmoney ) {
                                        Users.savedmoney++;
                                        Users.exportUserwealth();
